 ...
 */
async function searchUsers(query, queueID, csrfToken) {
    if (query.length < 2) {
    ...
    try {
        const response = await fetch(`/api/queue/${queueID}/search_users?q=${encodeURIComponent(query)}`, {
            method: 'GET',
            headers: {
        ...
        renderSearchResults(users, queueID, csrfToken);
    } catch (error) {
 ...
 * @param {string} csrfToken - The CSRF token
 */
function renderSearchResults(users, queueID, csrfToken) {
...
function addStaffToList(user) {
    ...
    listItem.innerHTML = `
        <span style="display: flex; align-items: center;">
            <input type="checkbox" class="admin-toggle" data-account-id="${user.id}" ${isAdminChecked} style="margin-right: 10px;">
...
function initStaffManagement(queueID, csrfToken) {
    ...
    if (staffList) {
        ...
        // NEW: Listener for toggling admin status
